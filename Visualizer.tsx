...
const Visualizer: React.FC<Props> = ({ items }) => {
  ...
  return (
    <div className="bg-white rounded-[2.5rem] p-8 border border-slate-200 shadow-sm mb-10 flex flex-col">
      <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-6">Распределение затрат по разделам</h3>
      
      {/* Сегментированная полоса */}
      <div className="w-full h-10 bg-slate-100 rounded-2xl flex overflow-hidden mb-8 shadow-inner">
        {data.map((entry, index) => (
          <div 
            key={`segment-${entry.name}`}
            style={{ width: `${entry.percent}%`, backgroundColor: COLORS[index % COLORS.length] }}
            className="h-full transition-all duration-500 hover:brightness-110 cursor-pointer relative group"
            title={`${entry.name}: ${entry.value.toLocaleString()} ₽ (${entry.percent.toFixed(1)}%)`}
          >
            {/* Тултип при наведении на сегмент */}
            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 bg-slate-900 text-white text-[10px] rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-50 pointer-events-none font-bold">
          ...
          <div key={entry.name} className="flex items-center justify-between group py-1">
            <div className="flex items-center gap-3 overflow-hidden">
              ...
              <div className="flex flex-col overflow-hidden">
                <span className="text-slate-700 font-bold text-xs truncate" title={entry.name}>
                  {entry.name}
                </span>
                <span className="text-[10px] text-slate-400 font-medium">
                  {entry.percent.toFixed(1)}% от общей суммы
                </span>
              </div>
            </div>
